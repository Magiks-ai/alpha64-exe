const data = window.MEMECOIN_RADAR_DATA || {candidates:[], counts:{}, source:{}};
    ...
    function dexPairToSnapshot(p,c,source='dexscreener live'){
      return {source, price:Number(p.priceUsd||c.priceUsd||0), liquidity:Number(p.liquidity?.usd||c.liquidityUsd||0), volume24:Number(p.volume?.h24||c.volume?.h24||0), tx5:Number(p.txns?.m5?.buys||0)+Number(p.txns?.m5?.sells||0), tx1h:Number(p.txns?.h1?.buys||0)+Number(p.txns?.h1?.sells||0), change5:Number(p.priceChange?.m5||0), change1h:Number(p.priceChange?.h1||0), change24:Number(p.priceChange?.h24||0), points:null};
    }
    function hydrateDexCandidate(c,p){
      const b=p.baseToken||{}, info=p.info||{};
      c.chainId=p.chainId||c.chainId; c.dexId=p.dexId||c.dexId; c.pairAddress=p.pairAddress||c.pairAddress; c.pairUrl=p.url||c.pairUrl;
      c.tokenAddress=b.address||c.tokenAddress; c.name=b.name||c.name; c.symbol=b.symbol||c.symbol; c.priceUsd=p.priceUsd||c.priceUsd;
      c.liquidityUsd=p.liquidity?.usd||c.liquidityUsd; c.volume=p.volume||c.volume||{}; c.txns=p.txns||c.txns||{}; c.priceChange=p.priceChange||c.priceChange||{};
      c.image=info.imageUrl||c.image;
      return c;
    }
    function bestDexPair(pairs,c){
      const ca=tokenCa(c).toLowerCase(); const sym=String(c.symbol||'').replace(/^\$/,'').toLowerCase();
      const scored=(pairs||[]).map(p=>{ const b=p.baseToken||{}, q=p.quoteToken||{}; let score=Number(p.liquidity?.usd||0); if(ca && [b.address,q.address].map(x=>String(x||'').toLowerCase()).includes(ca)) score+=1e12; if(sym && String(b.symbol||'').toLowerCase()===sym) score+=1e9; if(String(p.chainId||'').toLowerCase()==='solana') score+=5e6; return {p,score}; });
      scored.sort((a,b)=>b.score-a.score); return scored[0]?.p||null;
    }
    async function lookupDexPair(c){
      const terms=[...(c.dexSearchTerms||[]), tokenCa(c), c.symbol, c.name].map(x=>String(x||'').replace(/^\$/,'').trim()).filter(Boolean);
      for(const term of [...new Set(terms)]){
        try{
          const r=await fetch(`https://api.dexscreener.com/latest/dex/search?q=${encodeURIComponent(term)}`, {cache:'no-store'});
          if(!r.ok) continue;
          const j=await r.json(); const p=bestDexPair(j.pairs||[], c);
          if(p) return p;
        }catch(e){}
      }
      return null;
    ...
    async function fetchDexSnapshot(c){
      ...
      if(c.chainId && c.pairAddress){
        try{
          ...
          if(r.ok){ const j=await r.json(); const p=(j.pairs||[])[0]; if(p){ hydrateDexCandidate(c,p); return dexPairToSnapshot(p,c,'dexscreener live'); } }
        }catch(e){}
      }
      const found=await lookupDexPair(c);
      if(found){ hydrateDexCandidate(c,found); return dexPairToSnapshot(found,c,'dexscreener x-ticker lookup'); }
    ...
    async function openDexViewer(c){
      ...
      $('#dexTitle').textContent=`${c.name||'Token'} ${c.symbol?('$'+String(c.symbol).replace(/^\$/,'')):''}`; $('#dexMeta').textContent='opening market tape…';
      const ca=tokenCa(c); const copy=$('#dexCopyCa'); copy.dataset.ca=ca; copy.disabled=!ca; copy.querySelector('code').textContent=ca?shortCa(ca):'lookup'; copy.querySelector('b').textContent=ca?'copy':'wait';
      $('#dexExternal').href=safeUrl(c.pairUrl||'#'); $('#dexExternal').textContent=c.pairUrl?'open dexscreener ↗':'resolving from X ticker…';
      $('#dexRefresh').onclick=()=>openDexViewer(c);
      drawDexChart(c,pairSnapshot(c));
      try{ const snap=await fetchDexSnapshot(c); const liveCa=tokenCa(c); copy.dataset.ca=liveCa; copy.disabled=!liveCa; copy.querySelector('code').textContent=liveCa?shortCa(liveCa):'n/a'; copy.querySelector('b').textContent=liveCa?'copy':'none'; $('#dexTitle').textContent=`${c.name||'Token'} ${c.symbol?('$'+String(c.symbol).replace(/^\$/,'')):''}`; $('#dexExternal').href=safeUrl(c.pairUrl||'#'); $('#dexExternal').textContent=c.pairUrl?'open dexscreener ↗':'source unavailable'; $('#dexMeta').textContent=`${snap.source} · price ${snap.price?('$'+snap.price.toPrecision(5)):'n/a'} · liq ${money(snap.liquidity)} · vol24 ${money(snap.volume24)} · 1h ${Number(snap.change1h||0).toFixed(2)}% · tx5 ${snap.tx5||0}`; $('#dexTape').innerHTML=[['price',snap.price?('$'+Number(snap.price).toPrecision(5)):'n/a'],['liquidity',money(snap.liquidity)],['24h volume',money(snap.volume24)],['5m tx',snap.tx5||0],['1h change',Number(snap.change1h||0).toFixed(2)+'%'],['24h change',Number(snap.change24||0).toFixed(2)+'%']].map(([k,v])=>`<span><b>${esc(v)}</b>${esc(k)}</span>`).join(''); drawDexChart(c,snap); }
    ...
    function installTokenActions(){
      document.addEventListener('click', e=>{
        ...
        if(dex){ e.preventDefault(); e.stopPropagation(); if(dex.dataset.launchIdx!=null){ const l=(launchData.launches||[])[Number(dex.dataset.launchIdx)] || selectedLaunch; if(l) openDexViewer(launchDexCandidate(l)); return; } const pair=dex.dataset.pair; const c=data.candidates.find(x=>x.pairAddress===pair) || selected; if(c) openDexViewer(c); return; }
      }, true);
    }
    ...
    function launchProjectTitle(l){ const p=String(l?.project||'Launch cluster'); return CA_RE_BROWSER.test(p) ? shortCa(p) : p; }
    const CA_RE_BROWSER=/^[1-9A-HJ-NP-Za-km-z]{32,44}$/;
    function launchTickerMarkup(l){ const project=String(l?.project||'').toUpperCase(); return (l.tickers||[]).filter(t=>String(t).toUpperCase()!==project).map(t=>`<span class="sym">${esc(t)}</span>`).join(' '); }
    function launchDexCandidate(l){ const title=launchProjectTitle(l); const tickers=(l.tickers||[]).filter(t=>String(t).toUpperCase()!=='$SOL'); const primary=String((tickers[0]||l.project||title)||'').replace(/^\$/,''); const ca=(l.contracts||[])[0]||''; return {name:title, symbol:primary, tokenAddress:ca, chainId:'', pairAddress:'', pairUrl:'', dexSearchTerms:[...(l.contracts||[]), ...tickers, l.project, title].filter(Boolean), volume:{}, txns:{}, priceChange:{}, sourceLaunch:l}; }
    function launchEmblem(l, small=false){ const speaker=(l.speakers||[])[0]?.handle || ''; const seed=(l.project||'')+speaker; const h=hashCode(seed)%360; return `<div class="launch-emblem${small?' small':''}" style="--h:${h}" title="${esc(launchProjectTitle(l)||'launch')} ${speaker?'/ '+esc(speaker):''}"><span class="emblem-core">${esc(initials(launchProjectTitle(l)))}</span><span class="emblem-pixels"><i></i><i></i><i></i><i></i><i></i><i></i></span></div>`; }
    ...
    function launchCard(l,i){ const sec=l.security||{}; const active=selectedLaunch===l?' active':''; const status=sec.verdict||'review'; const lead=(l.speakers||[])[0]?.handle || 'unknown speaker'; const title=launchProjectTitle(l); const tickerMarkup=launchTickerMarkup(l); const speakers=(l.speakers||[]).slice(0,4).map(s=>`<a class="speaker" href="${safeUrl(s.xUrl)}" target="_blank" rel="noopener noreferrer">${esc(s.handle)} <b>×${s.mentions||1}</b></a>`).join(''); const warns=(sec.warnings||[]).slice(0,3).map(w=>`<span class="badge warn">${esc(w.replaceAll('_',' '))}</span>`).join(''); const sig=(sec.signals||[]).slice(0,3).map(x=>`<span class="badge raid">${esc(x.replaceAll('_',' '))}</span>`).join(''); return `<article class="launch-card ${verdictClass(sec.verdict)}${launchRail(l.score)}${active}" data-idx="${i}">${launchEmblem(l)}<div class="launch-content"><div class="status-pill ${verdictClass(sec.verdict)}">${esc(status.replaceAll('-',' '))}</div><div class="launch-title">${esc(title)} ${tickerMarkup}</div><div class="launch-speaker-label">lead signal: ${esc(lead)}</div><div class="launch-meta"><span>mentions <b>${(l.mentions||[]).length}</b></span><span>speakers <b>${(l.speakers||[]).length}</b></span><span>validity <b>${sec.score||0}</b></span></div><div class="speaker-row">${speakers || '<span class="meta">no speakers parsed</span>'}</div><div class="badges">${sig}${warns}</div><div class="bar-label">chatter strength <b>${Math.round(l.score||0)}</b></div><div class="spark" title="launch chatter score"><i style="width:${Math.min(100,l.score||0)}%"></i></div><div class="ca-row"><button class="dex-open dex-launch-open" type="button" data-launch-idx="${i}">arcade dex</button></div></div></article>`; }
    function renderLaunchDetail(){ const l=selectedLaunch; if(!l){ $('#launchDetail').innerHTML='<div class="empty">Select a launch cluster.</div>'; return; } const sec=l.security||{}; const title=launchProjectTitle(l); const tickerMarkup=launchTickerMarkup(l); const speakers=(l.speakers||[]).map(s=>`<a class="speaker" href="${safeUrl(s.xUrl)}" target="_blank" rel="noopener noreferrer">${esc(s.handle)} <b>${s.mentions||1}</b></a>`).join(''); const mentions=(l.mentions||[]).slice(0,6).map(m=>`<div class="launch-copy"><div class="mention-head"><b>${esc(m.author||'unknown')}</b>${m.tweetUrl?`<a class="tweet-link" href="${safeUrl(m.tweetUrl)}" target="_blank" rel="noopener noreferrer">open tweet ↗</a>`:''}</div><div>${esc(m.text)}</div></div>`).join(''); const moreMentions=(l.mentions||[]).length>6 ? `<div class="meta source-more">+${(l.mentions||[]).length-6} more chatter snippets in raw data</div>` : ''; const priors=(l.priorTokens||[]).map(t=>`<a class="prior-token" href="${safeUrl(t.pairUrl)}" target="_blank" rel="noopener noreferrer">${esc(t.speaker)} previously linked to ${esc(t.symbol||'?')} / ${esc(t.name||'?')} · ${esc(t.chainId||'')}</a>`).join('') || '<span class="meta">No previous Dexscreener-linked launches found for parsed speakers.</span>'; const urls=(l.urls||[]).slice(0,12).map((u,i)=>`<a href="${safeUrl(u)}" target="_blank" rel="noopener noreferrer">${esc(linkLabel(u,i))}</a>`).join(''); const moreUrls=(l.urls||[]).length>12 ? `<span class="meta source-more">+${(l.urls||[]).length-12} more source links in raw data</span>` : ''; $('#launchDetail').innerHTML=`<span class="inspecting-label">currently inspecting</span><div class="detail-emblem-row">${launchEmblem(l,true)}<div><h2>${esc(title)} ${tickerMarkup}</h2><p class="meta">Validity: <span class="${secClass(sec.verdict)}">${esc(sec.verdict||'review')}</span> · score ${sec.score||0} · launch-chatter score ${Math.round(l.score||0)}</p><div class="token-action-grid"><button class="dex-open dex-launch-open detail-dex" type="button" data-launch-idx="${(launchData.launches||[]).indexOf(l)}">open arcade dex viewer</button></div></div></div><h3>People talking about it</h3><div class="speaker-row">${speakers || '<span class="meta">No X accounts parsed.</span>'}</div><h3>Security / validity check</h3><div class="badges">${(sec.signals||[]).map(x=>`<span class="badge raid">${esc(x.replaceAll('_',' '))}</span>`).join('')} ${(sec.warnings||[]).map(x=>`<span class="badge warn">${esc(x.replaceAll('_',' '))}</span>`).join('')}</div><h3>Prior launches by speakers</h3>${priors}<h3>Project / launch links</h3><div class="links">${urls || '<span class="meta">No project links parsed from tweets.</span>'}${moreUrls}</div><h3>Recent launch chatter</h3>${mentions || '<span class="meta">No mention snippets.</span>'}${moreMentions}`; }
