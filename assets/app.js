const data = window.MEMECOIN_RADAR_DATA || {candidates:[], counts:{}, source:{}};
    ...
    function tokenId(c){ return tokenCa(c) || c?.pairAddress || `${c?.chainId||''}:${c?.symbol||''}`; }
    const WATCH_KEY='alpha64_watchlist_v1';
    function watchSet(){ try{return new Set(JSON.parse(localStorage.getItem(WATCH_KEY)||'[]'));}catch(e){return new Set();} }
    function saveWatchSet(set){ try{localStorage.setItem(WATCH_KEY, JSON.stringify([...set].slice(0,128)));}catch(e){} }
    function isWatched(c){ return watchSet().has(tokenId(c)); }
    function watchLabel(c){ return isWatched(c)?'WATCHED':'WATCH'; }
    function toggleWatch(c){ const id=tokenId(c); if(!id) return false; const set=watchSet(); const on=!set.has(id); on?set.add(id):set.delete(id); saveWatchSet(set); alphaToast(`${on?'WATCHED':'UNWATCHED'} // ${c.symbol||c.name||shortCa(id)}`, on?'ok':'warn'); mascotSay?.(on?`${c.symbol||c.name}: pinned to local watchlist.`:`${c.symbol||c.name}: removed from watchlist.`, on?'hype':'idle'); render(); return on; }
    ...
    function card(c,i){ const vol=money(c.volume?.h24), liq=money(c.liquidityUsd), watched=isWatched(c), active=selected&&selected.pairAddress===c.pairAddress?' active':'', raids=(c.raidSignals||[]).map(r=>`<span class="badge raid">${esc(r.replaceAll('_',' '))}</span>`).join(''), warns=(c.warnings||[]).map(w=>`<span class="badge warn">${esc(w.replaceAll('_',' '))}</span>`).join(''); return `<article class="card${pressureRail(c.score)}${active}${watched?' watched':''}" data-id="${esc(c.pairAddress)}" aria-label="Inspect ${esc(c.name||'token')} ${Math.round(c.score||0)} pressure"><div class="rank">#${String((i||0)+1).padStart(2,'0')}</div><img class="icon" src="${safeUrl(c.image)}" onerror="this.style.visibility='hidden'"><div><div class="name">${esc(c.name||'Unknown')} <span class="sym">$${esc(c.symbol||'?')}</span>${watched?'<span class="watch-chip">WATCHED</span>':''}</div><div class="meta ticker-meta"><span>${esc(c.chainId)}</span><span>${esc(c.dexId||'dex')}</span><span>age ${age(c.ageHours)}</span><span>vol ${vol}</span><span>liq ${liq}</span><span>h1 tx ${tx(c,'h1')}</span></div><div class="ca-row">${caMarkup(c,true)}<button class="watch-toggle" type="button" data-pair="${esc(c.pairAddress)}">${watchLabel(c)}</button><button class="share-signal" type="button" data-pair="${esc(c.pairAddress)}">share</button><button class="dex-open" type="button" data-pair="${esc(c.pairAddress)}">arcade dex</button></div><div class="badges"><span class="badge">${(c.contactTargets||[]).length} contacts</span>${raids}${warns}</div><div class="bar-label pressure-scale">pressure scale <b>${Math.round(c.score||0)} / 250</b></div><div class="spark" title="pressure score scaled against 250 cap"><i style="width:${pressurePct(c.score)}%"></i></div></div><div class="score"><span>${Math.round(c.score||0)}</span></div></article>`; }
    ...
    function installTokenActions(){
      document.addEventListener('click', e=>{
        ...
        const watch=e.target.closest('.watch-toggle');
        if(watch){ e.preventDefault(); e.stopPropagation(); const c=data.candidates.find(x=>x.pairAddress===watch.dataset.pair) || selected; if(c) toggleWatch(c); return; }
